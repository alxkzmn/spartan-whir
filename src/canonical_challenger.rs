...
/// Local copy of `SerializingChallenger32` whose field observation uses canonical
/// u32 encoding instead of `to_unique_u32()`.
#[derive(Clone, Debug)]
pub struct CanonicalSerializingChallenger32<F, Inner> {
    inner: Inner,
    _marker: PhantomData<F>,
}

impl<F: PrimeField32, Inner: CanObserve<u8>> CanonicalSerializingChallenger32<F, Inner> {
...
impl<F, H> CanonicalSerializingChallenger32<F, HashChallenger<u8, H, 32>>
where
    F: PrimeField32,
...
impl<F: PrimeField32, Inner: CanObserve<u8>> CanObserve<F>
    for CanonicalSerializingChallenger32<F, Inner>
{
...
impl<F, EF, Inner> CanSample<EF> for CanonicalSerializingChallenger32<F, Inner>
where
    F: PrimeField32,
...
impl<F, Inner> CanSampleBits<usize> for CanonicalSerializingChallenger32<F, Inner>
where
    F: PrimeField32,
    Inner: CanSample<u8>,
{
...
impl<F, Inner> GrindingChallenger for CanonicalSerializingChallenger32<F, Inner>
where
    F: PrimeField32,
...
impl<F, Inner> FieldChallenger<F> for CanonicalSerializingChallenger32<F, Inner>
where
    F: PrimeField32,
    Inner: CanSample<u8> + CanObserve<u8> + Clone + Send + Sync,
{
