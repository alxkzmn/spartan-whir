...
include "../koalabear-sha256/ch.circom";
include "maj.circom";

function AddedConstantBit(round, kind, bit) {
    if (kind == 0) {
        return Kbit(round, bit);
    }
    return bit == 0;
}

template BinSum32RoundConstantWide(round, ops, constant_kind) {
    ...
    if (ops <= 6) {
        ...
        for (var segment = 0; segment < 2; segment++) {
            ...
            for (var bit = 0; bit < segment_width; bit++) {
                kbit = AddedConstantBit(round, constant_kind, segment_start + bit);
            ...
            var constant_max = segment_base - 1;
            if (constant_kind == 1) {
                constant_max = segment == 0;
            }
            max_carry = (max_carry + ops * (segment_base - 1) + constant_max) \ segment_base;
            carry_width = 1;
            if (max_carry > 1) {
                carry_width = 2;
            }
        ...
        for (var k = 0; k < 32; k++) {
            kbit = AddedConstantBit(round, constant_kind, k);
            bit_sum = kbit;
            for (var j = 0; j < ops; j++) {
                bit_sum += in[j][k];
            }
...
template Sha256Round(round) {
    ...
    component sum_e = BinSum32RoundConstantWide(round, 5, 0);
    component sum_a = BinSum32RoundConstantWide(round, 4, 1);
    ...
    for (var k = 0; k < 32; k++) {
        sum_a.in[0][k] <== sum_e.out[k];
        sum_a.in[1][k] <== bigsigma0.out[k];
        sum_a.in[2][k] <== maj.out[k];
        sum_a.in[3][k] <== 1 - d[k];
    }
