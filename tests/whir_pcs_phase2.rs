...
use spartan_whir::{
    observe_whir_fs_domain_separator, KoalaExtension, KoalaField, KoalaKeccakChallenger,
    KoalaKeccakCompress, KoalaKeccakEngine, KoalaKeccakFieldHash, MlePcs, MultilinearPoint,
    PcsStatementBuilder, PointEvalClaim, SecurityConfig, SoundnessAssumption, SpartanWhirError,
    WhirParams, WhirPcs, WhirPcsConfig,
};
use whir_p3::{
...
fn sample_poly(num_variables: usize) -> Vec<KoalaField> {
    (0..(1 << num_variables))
        .map(|i| KoalaField::from_u32((i + 1) as u32))
        .collect()
}
...
fn claim_from_seed(
    ...
    num_variables: usize,
    seed: u32,
) -> (MultilinearPoint<KoalaExtension>, KoalaExtension) {
    let point = WhirPoint::expand_from_univariate(
        KoalaExtension::from(KoalaField::from_u32(seed)),
        num_variables,
    );
...
fn statement_with_seeds(
    ...
    num_variables: usize,
    seeds: &[u32],
) -> spartan_whir::PcsStatement<KoalaKeccakEngine> {
    let mut builder = PcsStatementBuilder::<KoalaKeccakEngine>::new();
    for seed in seeds {
...
fn whir_pcs_wrong_eval_fails() {
    ...
    let wrong_statement = PcsStatementBuilder::<KoalaKeccakEngine>::new()
        .add_point_eval(PointEvalClaim {
            point,
            value: eval + KoalaExtension::ONE,
        })
        .finalize()
        .unwrap();
...
fn whir_pcs_rejects_non_power_of_two_poly() {
    let config = test_config(6);
    let bad_poly = vec![
        KoalaField::from_u32(1),
        KoalaField::from_u32(2),
        KoalaField::from_u32(3),
    ];
    let mut challenger = spartan_whir::new_koala_keccak_challenger();
...
fn whir_pcs_rejects_linear_constraints_phase2() {
    let config = test_config(6);
    let poly = sample_poly(config.num_variables);
    let linear_statement = PcsStatementBuilder::<KoalaKeccakEngine>::new()
        .add_linear_constraint(spartan_whir::LinearConstraintClaim {
            coefficients: vec![KoalaField::from_u32(1)],
            expected: KoalaExtension::from(KoalaField::from_u32(1)),
        })
        .finalize()
        .unwrap();
...
fn whir_pcs_transcript_checkpoint_match() {
    ...
    let (commitment, prover_data) =
        WhirPcs::commit(&config, &poly, &mut prover_challenger).unwrap();
    let proof = WhirPcs::open(&config, prover_data, &statement, &mut prover_challenger).unwrap();
    let checkpoint_prover: KoalaExtension = prover_challenger.sample_algebra_element();
    ...
    let checkpoint_verifier: KoalaExtension = verifier_challenger.sample_algebra_element();
...
fn whir_pcs_domain_separator_roundtrip_alignment() {
    ...
    let mut adapter_challenger = spartan_whir::new_koala_keccak_challenger();
    observe_whir_fs_domain_separator(&config, &mut adapter_challenger).unwrap();
    let adapter_sample: KoalaField = adapter_challenger.sample();
    ...
    let protocol_params = ProtocolParameters {
        ...
        merkle_hash: KoalaKeccakFieldHash::for_security_bits(
            config.security.merkle_security_bits as usize,
        ),
        merkle_compress: KoalaKeccakCompress::for_security_bits(
            config.security.merkle_security_bits as usize,
        ),
    };
    ...
    >::new(config.num_variables, protocol_params);

    let mut manual_challenger = spartan_whir::new_koala_keccak_challenger();
    ...
    let manual_sample: KoalaField = manual_challenger.sample();
